 ...
 *
 * ## Алфавит (160 символов)
 *   Латиница (a-z, A-Z), кириллица (а-я, А-Я, включая ё/Ё),
 *   цифры (0-9), пробел, основные знаки препинания и спецсимволы, перевод строки.
 */
...
export default class CompactCipher {
    ...
    async encrypt(plaintext, password, chatId = '') {
        ...
        // 1. Валидация: все символы должны быть в алфавите
        ...
        // 2. Derive key (с кэшированием)
        const key = await this._deriveKey(password, chatId);

        // 3. Generate nonce (6 случайных индексов алфавита)
        ...
        // 4. Compute MAC: HMAC-SHA256(key, "MAC" || nonce || plaintext) → 6 цифр
        const macStr = await this._computeMAC(key, nonceIndices, plaintext);

        // 5. Объединить plaintext + MAC, затем зашифровать
        ...
        // 6. Сгенерировать ключевой поток и зашифровать
    ...
    async decrypt(ciphertext, password, chatId = '') {
        ...
        // 7. Преобразовать индексы в строку
        let decrypted = '';
